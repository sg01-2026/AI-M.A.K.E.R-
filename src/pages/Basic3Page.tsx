...
export default function Basic3Page() {
  const navigate = useNavigate();

  useEffect(() => {
    navigate('/basics?category=기본3(고급)', { replace: true });
  }, [navigate]);
  ...
  useEffect(() => {
    // Query resources matching Level 3
    const q = query(
      collection(db, 'resources'),
      where('category', 'in', ['기본3(고급)', '기본3', '기본3 (고급)', '기본3(심화)'])
    );
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <div className="flex flex-col items-center text-center space-y-6">
            ...
            <div className="space-y-2">
              <span className="text-emerald-400 font-serif tracking-[0.3em] text-[10px] uppercase font-bold block whitespace-nowrap break-keep">
                BASIC 3
              </span>
            ...
            <p className="text-zinc-300 font-sans max-w-2xl mx-auto leading-relaxed text-sm md:text-base break-keep">
              기본3 학습 내용을 차시별로 확인하고 학습 활동에 참여할 수 있습니다.
            </p>
          </div>
        </div>
      </section>
                ...
                >
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-mono bg-emerald-50 text-emerald-700 font-bold px-2 py-0.5 rounded-sm whitespace-nowrap break-keep">
                        {item.makerStage || '기본'}
                      </span>
                      {item.toolType && (
