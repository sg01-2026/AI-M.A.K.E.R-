...
import { MapPin, MessageSquare, BookOpen, PenTool, Share2, ChevronRight, Image as ImageIcon, FileText, Play, ExternalLink } from 'lucide-react';
           ...
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-16">
              <div className="space-y-12">
                 <div className="space-y-6">
                    <h2 className="text-3xl font-serif text-ink-900 pb-2 border-b border-gold-500/20">문화유산 만나기</h2>
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       {data.meet.records.map((r, i) => (
                         <div key={i} className="aspect-video bg-white overflow-hidden border border-gold-500/10 group cursor-pointer">
                            <img src={`https://picsum.photos/seed/${data.id}-${i}/400/300`} className="w-full h-full object-cover transition-transform group-hover:scale-105" alt="record" referrerPolicy="no-referrer" />
                            <div className="p-2 text-[10px] uppercase font-serif text-ink-800/50">{r}</div>
                         </div>
                       ))}
                    </div>
                 </div>
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                       {data.meet.stories.map((s, i) => (
                         <div key={i} className="p-6 bg-white border border-gold-500/5 hover:border-gold-500/20 transition-all font-serif">
                            {s}
                         </div>
                       ))}
                    </div>
                         ...
                         {resources.map((res) => (
                           ...
                           >
                             <div className="flex items-center space-x-4">
                               {res.type === 'image' ? <ImageIcon className="w-5 h-5 text-gold-600" /> : <FileText className="w-5 h-5 text-gold-600" />}
                               <div>
                                 <h4 className="font-serif text-ink-900">{res.title}</h4>
                                 <p className="text-xs text-ink-800/40">{res.type === 'image' ? '이미지 파일' : 'PDF 문서'}</p>
                               </div>
                             </div>
         ...
         {activeTab === 'ask' && (
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-20">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                 {[
                   ...
                   <div key={i} className="p-10 bg-white border border-gold-500/10 space-y-4 font-serif">
                      <div className="text-xs text-gold-600/50 uppercase tracking-widest">{item.label}</div>
              ...
              <div className="space-y-8 max-w-2xl mx-auto">
                 <h2 className="text-3xl font-serif text-center text-ink-900">학생들의 열린 질문</h2>
                 <div className="space-y-4">
                    {data.ask.studentQuestions.map((q, i) => (
                      <div key={i} className="flex items-start space-x-4 p-6 bg-gold-500/5 rounded-full px-12 border border-gold-500/10">
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                       {data.know.resources.map((r, i) => (
                         <div key={i} className="flex items-center justify-between p-4 bg-white border-b border-gold-500/10">
         ...
         {activeTab === 'express' && (
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-16">
              <div className="text-center space-y-4 max-w-2xl mx-auto mb-16">
                 ...
                 <p className="text-ink-800/60 font-serif">학생들이 AI와 함께 표현한 {data.name}의 새로운 이야기들입니다.</p>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                 {[1,2,3,4,5,6].map((grade) => (
                   <div key={grade} className="flex flex-col bg-white border border-gold-500/10 overflow-hidden group hover:border-gold-500/40 transition-all">
                      <div className="p-4 border-b border-gold-500/10 flex justify-between items-center bg-hanji-50">
                         <span className="text-xs font-serif uppercase tracking-widest text-gold-600 font-bold">{grade}학년</span>
                         <PenTool className="w-3 h-3 text-gold-600 opacity-50" />
                      </div>
                      <div className="aspect-[4/3] bg-hanji-100 overflow-hidden relative">
                         <img src={`https://picsum.photos/seed/${data.id}-grade-${grade}/600/450`} className="w-full h-full object-cover opacity-80" alt="work" referrerPolicy="no-referrer" />
           ...
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-16">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                 <div className="p-12 border-2 border-gold-500/20 bg-white font-serif space-y-6">
                    ...
                    <p className="text-ink-800/70 leading-relaxed">마을 주민들과 학생들이 함께 나누는 {data.name}의 추억과 미래.</p>
                    <button className="px-6 py-3 bg-gold-500 text-ink-900 text-sm hover:bg-gold-600 transition-colors">참여 기록 보기</button>
                 </div>
                 <div className="p-12 border-2 border-gold-500/20 bg-white font-serif space-y-6">
                    <h3 className="text-2xl text-gold-600">미래 시흥 이야기 이어가기</h3>
                    <p className="text-ink-800/70 leading-relaxed">100년 후의 {data.name}은 어떤 모습일까? AI가 그린 미래 청사진.</p>
                    <button className="px-6 py-3 bg-ink-900 text-hanji-100 text-sm hover:bg-ink-800 transition-colors">미래 아카이브</button>
                 </div>
              </div>
              ...
              <div className="py-20 text-center">
                 ...
                 <h2 className="text-4xl font-serif text-ink-900 mb-8">역사의 이야기는 <span className="italic underline decoration-gold-500/40">지금</span> 시작됩니다.</h2>
                 <Link to="/" className="text-ink-800/40 hover:text-gold-600 font-serif border-b border-dotted border-ink-800/20 pb-1 italic transition-all">시흥문화유산 M.A.K.E.R 가기</Link>
              </div>
           </motion.div>
         )}
      </section>
